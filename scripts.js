...
gsap.utils.toArray(".blur-text .word").forEach((element) => {
  gsap.fromTo(element, { filter: "blur(10px)", opacity: 0 }, { filter: "blur(0px)", opacity: 1, duration: 1.5, stagger: 0.1, scrollTrigger: {
    trigger: element,
    start: "top 80%", 
...
gsap.utils.toArray(".fade-in .word").forEach((element) => {
  gsap.fromTo(element, { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 1.5, stagger: 0.1, scrollTrigger: {
    trigger: element,
    start: "top 80%",
    toggleActions: "play none none none"
  }});
